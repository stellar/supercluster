...
type ShName =
    ...
    override self.ToString() =
        match self with
        | ShName s ->
            if ShName.IsName s then
                s
            else
                failwith ("ShName used with non-Name '" + s + "'")
...
type ShWord =
    ...
    static member OfStr(s: string) : ShWord =
        if ShPiece.IsBare s then
            ShPieces [| ShBare s |]
        elif s.StartsWith "${" && s.EndsWith "}" then
            ShPieces [| ShVar(ShName(s.Substring(2, (s.Length - 3)))) |]
        else
            ShQuoted s
    ...
    override self.ToString() =
        ...
        | ShPieces ps -> "\"" + String.concat "" (Array.map (sprintf "%O") ps) + "\""
        | ShQuoted s ->
            if s.Contains("'") then
                failwith ("ShQuoted used with inner quote '" + s + "'")
            else
                "'" + s + "'"
...
type ShCmd =
    ...
    | ShIf of (ShCmd * ShCmd * (ShCmd * ShCmd) array * ShCmd option) // else cmd;
    // fi

    static member True() : ShCmd = ShCmd [| ShWord.OfStr "true" |]

    static member OfStr(s: string) : ShCmd = ShCmd [| ShWord.OfStr s |]

    static member OfStrs(ss: string array) : ShCmd = ShCmd(Array.map ShWord.OfStr ss)

    static member DefVarSub (s: string) (ss: string array) : ShCmd = ShDefSub(ShName s, Array.map ShWord.OfStr ss)

    static member ExDefVar (s: string) (ss: string) : ShCmd = ShExDef(ShName s, ShPieces [| (ShBare ss) |])

    static member IfThen (i: string array) (t: string array) : ShCmd =
        ShIf((ShCmd.OfStrs i), (ShCmd.OfStrs t), [||], None)

    static member While (w: string array) (d: string array) : ShCmd = ShWhile((ShCmd.OfStrs w), (ShCmd.OfStrs d))

    static member Until (w: string array) (d: string array) : ShCmd = ShUntil((ShCmd.OfStrs w), (ShCmd.OfStrs d))
    ...
    member self.StdOutTo(f: string) : ShCmd = ShRedir(self, (ShRedirFdTo(1, ShWord.OfStr f)))

    member self.StdErrTo(f: string) : ShCmd = ShRedir(self, (ShRedirFdTo(2, ShWord.OfStr f)))

    member self.StdErrToOut(f: string) : ShCmd = ShRedir(self, (ShRedirFdDupTo(2, 1)))

    member self.StdInFrom(f: string) : ShCmd = ShRedir(self, (ShRedirFdFrom(0, ShWord.OfStr f)))

    override self.ToString() =
        let objs os = (Array.map (sprintf "%O") os)
        let words ws = String.concat " " (objs ws)
        let seq cs sep = "{ " + (String.concat sep (objs cs)) + "; }"
        ...
        | ShAnd cs -> seq cs " && "
        | ShOr cs -> seq cs " || "
        | ShWhile (c, d) -> sprintf "while %O; do %O; done" c d
        | ShUntil (u, d) -> sprintf "until %O; do %O; done" u d
        | ShFor (n, ws, d) -> sprintf "for %O in %s; do %O; done" n (words ws) d
        | ShIf (i, t, eis, eo) ->
