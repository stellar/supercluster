...
let MakeCatchupSets (options: CatchupMissionOptions) =
    let generatorOptions = { CoreSetOptions.GetDefault options.generatorImage with nodeCount = 1; quorumSet = Some(["generator"]); accelerateTime = true; }
    let generatorSet = MakeLiveCoreSet "generator" generatorOptions

    let newNodeOptions = { CoreSetOptions.GetDefault options.catchupImage with nodeCount = 1; quorumSet = Some(["generator"]); accelerateTime = true; initialization = CoreSetInitialization.DefaultNoForceSCP }
    ...
    let minimal2Options = { minimal1Options with quorumSet = Some(["minimal1"]); image = options.catchupImage }
    ...
    let complete2Options = { complete1Options with quorumSet = Some(["complete1"]); image = options.catchupImage }
    ...
    let recent2Options = { recent1Options with quorumSet = Some(["recent1"]); image = options.catchupImage }
    ...
    let minimal3Options = { minimal1Options with quorumSet = Some(["complete1"]); image = options.catchupImage }
    ...
    let recent3Options = { recent1Options with quorumSet = Some(["complete1"]); image = options.catchupImage }
    let recent3Set = MakeDeferredCoreSet "recent3" recent3Options

    let versionOptions = { CoreSetOptions.GetDefault options.versionImage with nodeCount = 1; quorumSet = Some(["version"]); accelerateTime = true; }
