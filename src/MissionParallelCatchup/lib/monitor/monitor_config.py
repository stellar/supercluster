"""Every knob the monitor owns, and the validation a run is admitted through.

What both processes must agree on -- the run's identity, the shared volume, the
verdict vocabulary -- lives in config, which this reads through.
...
`from monitor_config import REQ_CPU` binds a COPY. /start rebinds several of
these after validating them, and a copy taken at import time never sees the
rebind -- silently, with the run using the default.
...
def validate():
    ...
    if RANGE_ORDER not in RANGE_ORDERS:
        raise ValueError(f"RANGE_ORDER must be one of {RANGE_ORDERS}, "
                         f"got {RANGE_ORDER!r}")
    if config.STORAGE_MODE not in ('pvc', 'ephemeral'):
        raise ValueError(f"STORAGE_MODE must be pvc or ephemeral, "
                         f"got {config.STORAGE_MODE!r}")
