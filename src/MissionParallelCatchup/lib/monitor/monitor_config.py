"""Settings and run state for the parallel catchup job monitor.

Everything here belongs to the monitor process alone. What both processes must
agree on -- the run's identity, the shared volume, the verdict vocabulary --
lives in config, which this reads through for LOG_DIR.
...
`from monitor_config import REQ_CPU` binds a COPY. A test's monkeypatch and the
startup assignment of mc.PROFILE both rebind the attribute on this module, and a
