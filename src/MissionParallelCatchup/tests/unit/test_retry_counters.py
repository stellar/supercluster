...
def test_verdict_is_preferred_over_outcome(logdir):
    _write(jm.outcome_path(100, 1), {'outcome': 'disrupted'})
    _write(jm.verdict_path(100, 1), 'oom')
...
def test_matching_verdict_and_outcome_are_counted_once(logdir):
    _write(jm.outcome_path(100, 1), {'outcome': 'disrupted'})
    _write(jm.verdict_path(100, 1), 'disrupted')
...
def test_repeated_disruptions_of_one_range_count_as_one_retried_range(logdir):
    for attempt in (1, 2, 3):
        _write(jm.verdict_path(100, attempt), 'disrupted')
...
def test_disruptions_of_distinct_ranges_each_count_once(logdir):
    for end in (100, 200):
        _write(jm.outcome_path(end, 1), {'outcome': 'disrupted'})
        _write(jm.verdict_path(end, 1), 'disrupted')
...
def test_active_successor_counts_before_range_progress_exists(logdir):
    _write(jm.verdict_path(100, 1), 'rejected')
...
def test_counter_sync_is_idempotent_and_replays_after_restart(logdir):
    _write(jm.verdict_path(100, 1), 'disrupted')
    attempts = {('100', 2)}
    retry_before = jm.metric_retries._value.get()
    eviction_before = jm.metric_evictions._value.get()
    unique_before = jm.metric_spot_disruption_retried._value.get()
    reason_metric = jm.metric_retry_reasons.labels(reason='disrupted')
    reason_before = reason_metric._value.get()

    counted = {}
    jm.sync_counters({}, counted, attempts)
    first = (jm.metric_retries._value.get(),
             jm.metric_evictions._value.get(),
             jm.metric_spot_disruption_retried._value.get(),
             reason_metric._value.get())
    jm.sync_counters({}, counted, attempts)
    assert (jm.metric_retries._value.get(),
            jm.metric_evictions._value.get(),
            jm.metric_spot_disruption_retried._value.get(),
            reason_metric._value.get()) == first

    jm.sync_counters({}, {}, attempts)
    assert jm.metric_retries._value.get() == retry_before + 2
    assert jm.metric_evictions._value.get() == eviction_before + 2
    assert jm.metric_spot_disruption_retried._value.get() == unique_before + 2
    assert reason_metric._value.get() == reason_before + 2


def test_multiple_attempts_and_every_retry_reason(logdir):
    for attempt, reason in enumerate(jm.ATTEMPT_OUTCOMES, 1):
        _write(jm.verdict_path(100, attempt), reason)

    totals = _totals({'completed': {'100': {
        'attempts': len(jm.ATTEMPT_OUTCOMES) + 1}}})

    assert totals['retries'] == len(jm.ATTEMPT_OUTCOMES)
    assert totals['reasons'] == {reason: 1 for reason in jm.ATTEMPT_OUTCOMES}
    assert totals['evicted'] == 1
    assert totals['spot_disruption_retried'] == 1
    assert totals['oom'] == 1
...
def test_malformed_and_missing_records_do_not_invent_reasons(logdir):
    _write(jm.outcome_path(100, 1), {'outcome': 'disrupted'})
    _write(jm.verdict_path(100, 1), 'not-a-verdict')
    _write(jm.outcome_path(200, 1), 'not-json')
...
def test_existing_and_reason_labelled_metrics_are_exported():
    for reason in jm.ATTEMPT_OUTCOMES:
        jm.metric_retry_reasons.labels(reason=reason)
