...
# Sample query to gather tx_history data:
...
def process_history_row(row: dict[str, str]) -> Tuple[Optional[Tuple[int, int, int]], Optional[int]]:
    """
    Process a row from the tx_history table. Returns:
...
def process_soroban_history(tx_history_csv: str) -> None:
    """ Generate histograms from data in the tx_history table. """
    with open(tx_history_csv) as f:
...
def help_and_exit() -> None:
    print(f"Usage: {sys.argv[0]} <tx_history data> <history_contract_events data>")
